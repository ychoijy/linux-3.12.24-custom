...
retry_cpuset:
	...
	if (vma) {
		...
		/*
		if (mm && mm->sec_num) {
			printk("%s:%d mm->sec_num = %d\n",
			       __func__, __LINE__, mm->sec_num);
			for (i=0 ;i < mm->sec_num; i++) {
				if (!strcmp(mm->secinfos[i].name,
					    "DRAM_STATIC")) {
					if (mm->dram_from <= addr &&
					    mm->dram_to >= addr) {
						gfp |= __GFP_DRAM;
						printk("%s:%d address : %ld -> DRAM_ZONE",
